...
class MigrateFullMetadataUrl(CkanCommand):
    ...
    def __init__(self, name):
        ...
        self.parser.add_option('-s', '--source_url', dest='source_url', help='Remote CKAN source URL', type=str,
                               default=None)
        self.parser.add_option('-k', '--api_key', dest='api_key', help='Remote CKAN source API Key', type=str,
                               default=None)
        self.parser.add_option('-l', '--limit', dest='limit', help='Limit the number of datasets to process', type=int,
                               default=100)
        self.parser.add_option('-o', '--offset', dest='offset', help='Offset for the number of datasets to process',
                               type=int, default=0)
    ...
    def fetch_local_package(self, context, package_name):
        local_package = None
        try:
            local_package = toolkit.get_action('package_show')(context, {
                'id': package_name
            })
        except NotFound:
    ...
    def get_package_names(self, limit, offset):
        packages = toolkit.get_action('package_list')(data_dict={'limit': limit, 'offset': offset})
    ...
    def command(self):
        ...
        for package_name in package_names:
            ...
            if local_package.get('full_metadata_url', None):
                print('- Local package %s has `full_metadata_url` set to %s - nothing to do.' % (
                package_name, local_package.get('full_metadata_url')))
                continue

            print(
                        '- Local package name %s does not have existing `full_metadata_url` - attempting to fetch...' % package_name)

            # Fetch matching package by ID from remote source
            remote_package = self.fetch_remote_package(source_url, api_key, package_name)
            ...
            if not full_metadata_url:
                print('- `full_metadata_url` not found in remote package - nothing to do.')
                continue
